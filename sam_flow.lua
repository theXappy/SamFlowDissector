...
-- S*msungFlowFramework.NET.dll Creation Time: 29/04/2021 5:00:53 AM
-- S*msungFlowFramework.NET.dll Version: AuthVer: 11, AuthVerMin: 9, NotifVer: 11, NotifVerMin: 9
-- Dissector generation time: 1/05/2021 10:32:57 PM
--
--------------------------------------------------------------------------------------------------
...
function p_sam_flow_data.dissector(buf, pkt, tree)
	...
	file_name = data_channels_file_names[pkt.src_port]
	if not file_name then 
		-- table.remove is like Dequeue
		file_name = table.remove(data_channels_file_names_hints)
		-- Associate hinted file name with current TCP connection.
		-- Next time this function runs the name will be retrieved from data_channels_file_names
		data_channels_file_names[pkt.src_port] = file_name
	end
	subtree:add(f_data_file_name, file_name):set_generated()
	pkt.cols.info:set("Contents of file '" .. file_name .. "'")
...
function p_samsung_flow_priv.dissector(buf, pkt, tree)
		...
		local json_text = res
		generic_dissect_root(json_tvb, pkt, subtree, json_text, fm_finfos )
...
is_in_sockData = false
function socketServerInfoData_pre(buf, pkt, subtree, text, json_field_info, parsed_value, created_ti)
	is_in_sockData = true;
end
function socketServerInfoData_post(buf, pkt, subtree, text, json_field_info, parsed_value, created_ti)
	is_in_sockData = false;
end
function port_post(buf, pkt, subtree, text, json_field_info, parsed_value, created_ti)
	if is_in_sockData and parsed_value ~= nil then
		tcp_port_table = DissectorTable.get("tcp.port")
		tcp_port_table:add(parsed_value, p_sam_flow_data)
	end
end

-- 3 Functions below allow passing file name to data channel (different TCP connection from the signaling (private) channel we are on)
is_in_fileInfo = false;
function fileInfoData_pre(buf, pkt, subtree, text, json_field_info, parsed_value, created_ti)
	is_in_fileInfo = true;
end
function fileInfoData_post(buf, pkt, subtree, text, json_field_info, parsed_value, created_ti)
	is_in_fileInfo = false;
end
function fileName_post(buf, pkt, subtree, text, json_field_info, parsed_value, created_ti)
	if is_in_fileInfo and parsed_value ~= nil then
		-- table.insert is like Enqueue
		table.insert(data_channels_file_names_hints, parsed_value)
	end
end
